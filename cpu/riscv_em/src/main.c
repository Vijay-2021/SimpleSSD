...
void print_usage() {
    printf("Usage: riscv_em -f <firmware_file> [-d dtb_file] [-i initrd_file] [-s success_pc] [-n num_cycles]\n");
    printf("  -f <firmware_file>   : Path to the firmware file\n");
    printf("  [-d dtb_file]       : Path to the device tree blob file\n");
    printf("  [-i initrd_file]    : Path to the initrd file\n");
    printf("  [-s success_pc]     : Success PC address (in hex)\n");
    printf("  [-n num_cycles]     : Number of cycles to run\n");
}
                          ...
                          uint64_t *num_cycles)
{
    ...
    while ((c = getopt(argc, argv, "s:f:d:i:n:h:")) != -1)
    {
        switch (c)
        {
            case 's':
