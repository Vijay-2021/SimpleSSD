 ...
 */
 ...
 PageMapping::PageMapping(char* config_file, ext2_context* ctx, Parameter& p) : 
                          ssd_params(p), lastFreeBlock(param.pageCountToMaxPerf),
                          lastFreeBlockIOMap(param.ioUnitInPage), bReclaimMore(false) {
  
  blocks.reserve(ssd_params.totalPhysicalBlocks);
  table.reserve(ssd_params.totalLogicalBlocks * ssd_params.pagesInBlock);
  void *fw_file = ext_read_file(ctx, config_file);

  for (uint32_t i = 0; i < ssd_params.totalPhysicalBlocks; i++) {
    freeBlocks.emplace_back(Block(i, ssd_params.pagesInBlock, ssd_params.ioUnitInPage));
  }

  nFreeBlocks = ssd_params.totalPhysicalBlocks;

  status.totalLogicalPages = ssd_params.totalLogicalBlocks * ssd_params.pagesInBlock;
 ...
 bool PageMapping::initialize() {
   ...
   if (nPagesToWarmup + nPagesToInvalidate > maxPagesBeforeGC) {
     printf("ftl: Too high filling ratio. Adjusting invalidPageRatio.");
     nPagesToInvalidate = maxPagesBeforeGC - nPagesToWarmup;
   }
 
   printf("Total logical pages: %d", nTotalLogicalPages);
   debugprint(LOG_FTL_PAGE_MAPPING, "Total logical pages to fill: %" PRIu64 " (%.2f %%)",
