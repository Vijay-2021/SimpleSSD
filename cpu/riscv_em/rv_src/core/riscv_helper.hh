...
#include "riscv_xlen_specifics.hh"
...
static inline void assign_xlen_bit(rv_word_t *out_var, rv_word_t nbit, rv_word_t bit_value)
{
    *out_var = (*out_var & ~(1UL << nbit)) | ((bit_value & 1) << nbit);
}

static inline void assign_xlen_value_within_reg(rv_word_t *out_var, rv_word_t nbit, rv_word_t value, rv_word_t mask)
{
...
static inline rv_word_t extractxlen(rv_word_t value, int start, int length)
{
    return (value >> start) & (((rv_word_t)-1) >> ((sizeof(rv_word_t)*8) - length));
}
