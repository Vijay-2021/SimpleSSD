...
{
    pmp_td *pmp = priv;
    uint8_t cfg_reg_idx = reg_index/sizeof(rv_word_t);
    uint8_t cfg_idx = reg_index%sizeof(rv_word_t);
...
static inline rv_word_t get_pmp_napot_size_from_pmpaddr(rv_word_t addr)
{
    /* +2 because it was shifted by 2 with encoding */
    return (1 << (FIND_FIRST_BIT_SET(~addr) + 2) );
}

static inline rv_word_t get_pmp_napot_addr_from_pmpaddr(rv_word_t addr)
{
    rv_word_t size = get_pmp_napot_size_from_pmpaddr(addr);
    rv_word_t mask = ((size/2) - 1) >> 2;
    return (addr - mask) << 2;
}
...
rv_ret pmp_mem_check(pmp_td *pmp, privilege_level curr_priv, rv_word_t addr, uint8_t len, bus_access_type access_type)
{
    ...
    rv_word_t addr_end = 0;
    rv_word_t pmp_addr_start = 0;
    rv_word_t pmp_addr_size = 0;
        ...
        for(j=0;j<sizeof(pmp->cfg[0]);j++)
        {
            ...
            switch(addr_mode)
            {
                ...
                case pmp_a_napot:
                     ...
                     */
                    if(pmp->addr[addr_count] == (rv_word_t)-1)
                    {
                        pmp_addr_size = -1;
                        pmp_addr_start = 0;
                    }
