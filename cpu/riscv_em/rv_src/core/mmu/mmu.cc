...
rv_ret mmu_read_csr(void *priv, privilege_level curr_priv_mode, uint16_t reg_index, rv_word_t *out_val)
{
    (void)curr_priv_mode;
    (void) reg_index;
...
rv_ret mmu_write_csr(void *priv, privilege_level curr_priv, uint16_t reg_index, rv_word_t csr_val)
{
    (void) curr_priv;
    (void) reg_index;
...
// int translate_address(State * state, word virtual_address, enum access_type access_type, word * physical_address) {
	...
	//supervisor mode
		...
		// rv_word_t pte_addr = (mmu->satp_reg & 0x3FFFFF) * SV32_PAGE_SIZE;
		...
		for (int i = 0; i < levels; i++) {
            ...
            if(mmu->bus_access(mmu->priv, curr_priv, bus_read_access, pte_addr, &pte, sizeof(rv_word_t)) != rv_ok)
			...
			if (xwr != 0) {
				...
				// rv_word_t accessed = pte & _PAGE_ACCESSED;
				// rv_word_t dirty = (access_type == bus_write_access) * _PAGE_DIRTY;

				//update accessed flag on all accesses and dirty flag on store
				rv_word_t accessed_or_dirty_flags = _PAGE_ACCESSED | (access_type == bus_write_access? _PAGE_DIRTY : 0);
				if ((pte & accessed_or_dirty_flags) != accessed_or_dirty_flags) {
#ifdef ENABLE_DIRTY
					pte |= accessed_or_dirty_flags;
					//update pte 
					// write_word_physical(state, pte_addr, pte);
                    if(mmu->bus_access(mmu->priv, curr_priv, bus_write_access, pte_addr, &pte, sizeof(rv_word_t)) != rv_ok)
                        printf("mmu bus access err!\n");
				...
				rv_word_t vaddr_mask = ((rv_word_t)1 << vaddr_shift) - 1;
				//add the virtual address offset
				rv_word_t result = (virt_addr & vaddr_mask) | (paddr & ~vaddr_mask);
				*ret_val = mmu_ok;
				return result;
			}
    ...
    *ret_val = mmu_ok;
    ...
    for(i=(SV32_LEVELS-1),j=0;i>=0;i--,j++)
    {
        ...
        */
         ...
         */
        mmu->bus_access(mmu->priv, curr_priv, bus_read_access, pte_addr, &pte, sizeof(rv_word_t));
        ...
        {
            MMU_DEBUG("page fault: pte.v = 0, or if pte.r = 0 and pte.w = 1 access_type: %d a: "PRINTF_FMT" virt_addr: "PRINTF_FMT" pte: "PRINTF_FMT" pte_addr: "PRINTF_FMT" flags: %x curr_priv: %d level: %d value: "PRINTF_FMT"\n", access_type, a, virt_addr, pte, pte_addr, pte_flags, curr_priv, i, value);
            *ret_val = mmu_page_fault;
            return 0;
        }
