...
SimpleICL::ICL(icl_params& cparams, FTL::FTL *ftl) :
      pFTL(ftl),
      params(cparams),
...
uint32_t SimpleICL::calcSetIndex(uint64_t lca) {
  return lca % setSize;
}

void SimpleICL::calcIOPosition(uint64_t lca, uint32_t &row, uint32_t &col) {
...
uint32_t SimpleICL::getEmptyWay(uint32_t setIdx) {
...
uint32_t SimpleICL::getValidWay(uint64_t lca) {
...
void SimpleICL::checkSequential(Request &req, SequentialDetect &data) {
  ...
  if (useReadCaching) {
    ...
    if (wayIdx != waySize) {
      ...
      if (useReadPrefetch && req.range.slpn == prefetchTrigger) {
        ...
        mutex_lock(&icl_mutex); // lock mutex to prevent other threads from reading cache while we are prefetching
        goto ICL_GENERIC_CACHE_READ;
      } else {
        uint64_t tick;
        mutex_unlock(&icl_mutex);
        read_buffer((uint64_t)&tick, 0, FIRMWARE_TICK);
...
void SimpleICL::resetStatValues() {
  memset(&icl_stats, 0, sizeof(icl_stats));
}

uint32_t SimpleICL::evictFunction(uint32_t setIdx) {
  uint32_t wayIdx = 0;
...
Line* SimpleICL::compareFunction(Line *a, Line *b) {
  if (a && b) {
