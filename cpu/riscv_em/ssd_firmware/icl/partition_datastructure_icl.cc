...
PartitionedDSICL::PartitionedDSICL(icl_params& cparams, FTL::FTL *ftl) :
      AbstractICL(cparams, ftl) {
  printf("calling data structure locked icl initializer!\n");
  mutex_init(&cache_metadata_mutex);
  mutex_init(&cache_data_mutex);
  mutex_init(&ftl_mutex);
}
  

void PartitionedDSICL::evictCache(bool flush) {
...
// True when hit
bool PartitionedDSICL::read(Request &req) {
    bool ret = false;
...
bool PartitionedDSICL::write(Request &req) {
  uint64_t tick;
  uint64_t start_cycle;
  uint64_t end_cycle;
  bool ret = false;
...
// True when flushed
void PartitionedDSICL::flush(LPNRange &range) {
...
// True when hit
void PartitionedDSICL::trim(LPNRange &range) {
...
void PartitionedDSICL::format(LPNRange &range) {
    uint64_t start_cycle;
    uint64_t end_cycle;
    read_buffer((uint64_t)&start_cycle, 0, FIRMWARE_CYCLE);
    mutex_lock(&cache_metadata_mutex);
