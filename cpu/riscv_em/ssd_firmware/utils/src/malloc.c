...
bool init_allocator(const void *base, const void *limit, const size_t heap_blocks, const size_t split_thresh, const size_t alignment) {
    heap = (Heap *)base;
    heap_limit = limit;
...
void *calloc(size_t num, size_t size) {
    ...
    Block *block = alloc_block(num);
    if (block != NULL) {
        mutex_unlock(&ta_mutex); // block allocated, no need to hold mutex while zeroing memory
...
bool check() {
    return heap_max_blocks == num_free() + num_used() + num_fresh();
}
