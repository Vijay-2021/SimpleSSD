...
class EmbExt2 {
    private:
        ext2context *context;
    public: 
        EmbExt2(blockno_t part_start, blockno_t volume_size, uint8_t filesystem_hint);
        ~EmbExt2();
        void *open(const char *name, int flags, int mode); 
        int close(void *vfe);
        int read(void *vfe, void *buffer, uint32_t count);
        int write(void *vfe, const void *buffer, uint32_t count);
        int lseek(void *vfe, int ptr, int dir);
        int fstat(void *vfe, struct file_stats *stats);
}; 
